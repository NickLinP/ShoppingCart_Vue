          ...
          <ul>
            <li v-on:mouseenter="shopCartShow()" v-on:mouseleave="shopCartHidden()">
              ...
              <div style="
                  ...
                  position: fixed;
                  box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2);
                " class="border row p-3" v-show="shopCartNotEmpty">

                <div v-for="(item,index) in shopCartContent">{{item}}</div>

                <div class="col-3">
                  <img src="./image/homepage/hotItem/1_1-max-440.jpg" alt="圖片" style="width: 50px;" />
                </div>
                <div class="col-5">
                  <p style="font-size: 8px;">
                    Blue Bay 倍力 Animate 無穀貓飼料 室內貓/化毛配方-鮭魚+雞肉 2kg
                  </p>
                </div>
                <div class="col-1" style="font-size: 8px;">1</div>
                <div class="col-2" style="font-size: 8px;">NT600</div>
                <div class="col-1">
                  <a href="" style="font-size: 8px;display: block;text-align: top;color: black;">X</a>
                </div>
                <hr>
                <p style="text-align: right;">總計 : NT9999</p>
                <button type="button" class="btn btn-primary">前往結帳 ></button>
              </div>
...
<script>
  module.exports = {
    ...
    beforeCreate() {
    // 目前執行將Cookie的購物車紀錄帶入data內等待使用無效，因此先移到created內執行，問題等待釐清
    },
    created() {

      // 將Cookie的購物車紀錄帶入data內等待使用
        if (this.shopCartContent !== undefined) {
        this.shopCartContent = JSON.parse(Cookies.get("shopCartMemory"));
      }
      console.log(`購物車內容是 : ${this.shopCartContent}`);
